    ...
    <div className="min-h-screen bg-white text-slate-900">
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white shadow-xl py-2' : 'bg-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="flex items-center">
            <a href="/" className="flex items-center group">
              <motion.div whileHover={{ scale: 1.05 }} className="h-12 md:h-16 flex items-center justify-center overflow-hidden">
                <img src={LOGO_URL} alt="Grupo Limpeza" className="h-full w-auto object-contain" />
              </motion.div>
            </a>
          </motion.div>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link, i) => (
              <motion.a key={link.name} href={link.href} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} whileHover={{ y: -2, color: '#0070BC' }} className="text-slate-600 font-bold transition-colors text-sm">
                {link.name}
              </motion.a>
            ))}
            <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.05, boxShadow: "0 10px 20px -5px rgba(0, 112, 188, 0.4)" }} whileTap={{ scale: 0.95 }} onClick={() => openForm()} className="bg-brand-blue text-white px-7 py-2.5 rounded-full font-bold shadow-lg">
              Solicitar Cotação
            </motion.button>
          </div>
      ...
      <section className="relative pt-24 pb-12 md:pt-40 md:pb-16 px-4 bg-slate-50 overflow-hidden">
          ...
          {bgIcons.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                transition={{ duration: 8 + Math.random() * 4, repeat: Infinity, delay: item.delay, ease: "easeInOut" }}
              >
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          <motion.div initial={{ opacity: 0, x: -70 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, ease: "easeOut" }} className="space-y-8 mobile-center-all z-10">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-brand-blue text-white font-bold text-xs uppercase tracking-widest shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-5 mobile-w-full">
              <motion.button whileHover={{ scale: 1.05, boxShadow: "0 20px 25px -5px rgba(0, 112, 188, 0.3)" }} whileTap={{ scale: 0.95 }} onClick={() => openForm()} className="px-10 py-5 bg-brand-blue text-white rounded-2xl font-bold text-lg shadow-2xl shadow-brand-blue/30 w-full sm:w-auto">
                Solicitar Cotação Agora
              </motion.button>
              <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="https://wa.me/5521971250381" target="_blank" className="px-10 py-5 border-2 border-brand-blue/20 bg-white text-brand-blue rounded-2xl font-bold text-lg flex items-center justify-center gap-3 shadow-lg w-full sm:w-auto">
                <MessageCircle className="w-6 h-6" /> WhatsApp 24h
              </motion.a>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.8, x: 70 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 1, ease: "easeOut" }} className="relative">
            <div className="rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white bg-white">
              <motion.img whileHover={{ scale: 1.05 }} transition={{ duration: 0.5 }} src={HERO_IMAGE_URL} alt="Empresa de Limpeza Profissional" className="w-full aspect-square md:aspect-video lg:aspect-square object-cover" />
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-md p-5 rounded-2xl shadow-xl flex items-center gap-4">
      ...
      <section id="por-que-nos" className="py-12 px-4 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: <ShieldCheck className="w-10 h-10" />, title: "15+ Anos de Experiência", desc: "Atuamos há mais de uma década no Rio de Janeiro entregando soluções personalizadas." },
              { icon: <Award className="w-10 h-10" />, title: "+150 Limpezas Realizadas", desc: "Centenas de ambientes transformados e clientes que permanecem conosco." },
              { icon: <Users className="w-10 h-10" />, title: "Profissionais Qualificados", desc: "Nossa equipe passa por treinamento técnico contínuo e checklist supervisionado." }
            ].map((item, idx) => (
              <motion.div key={idx} variants={scaleIn} whileHover={{ y: -10 }} className="flex flex-col items-center text-center p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:shadow-2xl transition-all group">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <motion.div key={i} variants={scaleIn} whileHover={{ y: -10 }} className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col relative group">
                <div className="absolute top-8 right-10 text-brand-blue/30"><Quote size={32} /></div>
                <div className="flex gap-1 mb-6">
                  {[...Array(t.stars)].map((_, idx) => <Star key={idx} className="w-5 h-5 fill-yellow-400 text-yellow-400" />)}
                </div>
                <p className="text-slate-600 font-semibold italic mb-8 leading-relaxed relative z-10">"{t.content}"</p>
                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                  <div className="text-brand-blue font-black text-lg">{t.name}</div>
                  <div className="bg-brand-blue/5 px-3 py-1.5 rounded-full text-[10px] font-black text-brand-blue uppercase">{t.type}</div>
                </div>
              </motion.div>
            ))}
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.map((service) => (
              <motion.div key={service.id} variants={fadeInUp} whileHover={{ y: -10, scale: 1.01 }} className="bg-slate-50 rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col group items-center text-center">
                <motion.div className="mb-8 text-brand-blue p-5 bg-white rounded-2xl w-fit group-hover:bg-brand-blue group-hover:text-neon-green transition-all shadow-md mx-auto">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openForm(service.title)} className="w-full py-5 bg-brand-blue text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg">
      ...
      <footer className="bg-slate-50 border-t py-12 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <div className="mb-8 flex justify-center">
            <img src={LOGO_URL} alt="Grupo Limpeza" className="h-16 w-auto" />
          </div>
          <div className="flex justify-center gap-6 mb-8">
            <motion.a whileHover={{ scale: 1.2 }} href="https://www.instagram.com/grupolimpeza/" target="_blank" className="w-12 h-12 bg-brand-blue text-white rounded-xl flex items-center justify-center"><Instagram className="w-6 h-6" /></motion.a>
            <motion.a whileHover={{ scale: 1.2 }} href="https://wa.me/5521971250381" target="_blank" className="w-12 h-12 bg-brand-blue text-white rounded-xl flex items-center justify-center"><MessageCircle className="w-6 h-6" /></motion.a>
          </div>
          <div className="text-slate-400 font-black text-[10px] uppercase tracking-[0.4em]">Copyright © 2026 Grupo Limpeza</div>
        </div>
      </footer>

      <motion.a initial={{ scale: 0 }} animate={{ scale: 1 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} href="https://wa.me/5521971250381" target="_blank" className="fixed bottom-8 right-8 z-[90] bg-brand-blue text-white p-6 rounded-2xl shadow-2xl pulse-btn">
        <MessageCircle className="w-10 h-10" />
      </motion.a>

      <SmartForm key={selectedServiceForForm.join(',') || 'empty'} isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} initialServices={selectedServiceForForm} />
    </div>
  );
};
